...
class Progress(models.Model):
    ...
    def list_all_cat_scores(self):
        cat_scores = {}
        sittings = Sitting.objects.filter(user=self.user, complete=True)
        for sitting in sittings:
            category = sitting.quiz.category or "Uncategorized"
            if category not in cat_scores:
                cat_scores[category] = [0, 0, 0]
            total_questions = sitting.get_max_score
            correct = sitting.current_score
            incorrect = len(sitting.get_incorrect_questions)
...
class AIGenerationLog(models.Model):
    ...
    def __str__(self):
        return f"{self.user} - {self.topic} ({self.generated_at})"
