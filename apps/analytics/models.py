...
class ReportType(GlobalBaseModel):
    """
    ...
    category = models.CharField(
        _("category"), max_length=20, choices=ReportCategory.choices
    )
    ...
    parameters_schema = models.JSONField(
        _("parameters schema"), default=dict, blank=True
    )
...
class Report(CoreBaseModel):
    ...
    file = models.FileField(
        _("report file"), upload_to="reports/%Y/%m/%d/", null=True, blank=True
    )
    ...
    generation_started_at = models.DateTimeField(
        _("generation started at"), null=True, blank=True
    )
    generation_completed_at = models.DateTimeField(
        _("generation completed at"), null=True, blank=True
    )
    expires_at = models.DateTimeField(_("expires at"), null=True, blank=True)
    access_count = models.PositiveIntegerField(_("access count"), default=0)
    last_accessed_at = models.DateTimeField(
        _("last accessed at"), null=True, blank=True
    )
...
class Dashboard(CoreBaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        """
        if self.is_default:
            Dashboard.objects.filter(owner=self.owner, is_default=True).update(
                is_default=False
            )
        super().save(*args, **kwargs)


class KPI(GlobalBaseModel):
    """
    ...
    category = models.CharField(
        _("category"), max_length=20, choices=KPICategory.choices
    )
    description = models.TextField(_("description"), blank=True)
...
class KPIMeasurement(CoreBaseModel):
    ...
    previous_value = models.DecimalField(
        _("previous value"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    change_percentage = models.DecimalField(
        _("change percentage"), max_digits=6, decimal_places=2, null=True, blank=True
    )
...
class DataExport(CoreBaseModel):
    ...
    file = models.FileField(
        _("export file"), upload_to="exports/%Y/%m/%d/", null=True, blank=True
    )
...
class AnalyticsCache(CoreBaseModel):
    ...
    cache_key = models.CharField(
        _("cache key"), max_length=255, unique=True, db_index=True
    )
