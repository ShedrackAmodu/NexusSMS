...
from .models import (
    ...
    QuizAttempt, QuizProgress, CourseGrade, AIGenerationLog, TakenCourse
)
from .forms import (
...
def generate_ai_questions(request, question_bank_id):
    ...
    if not Class.objects.filter(
        ...
        messages.error(request, 'You do not have permission to modify this question bank.')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if created_questions:
                ...
                messages.warning(request, 'AI generation completed but no valid questions were created.')
...
def ai_generation_history(request):
    ...
    generation_logs = AIGenerationLog.objects.filter(
        user=teacher
    ).select_related('question_bank').order_by('-generated_at')
...
def quiz_marking_list(request):
    ...
    ).select_related('student__user', 'question_bank').order_by('-end_time')
...
def quiz_marking_detail(request, attempt_id):
    ...
    if not Class.objects.filter(
        ...
        messages.error(request, 'You do not have permission to grade this quiz attempt.')
        ...
        if question_id and marks:
            try:
                ...
                if str(question.id) in user_answers:
                    ...
                    user_answers[str(question.id)]['graded_at'] = timezone.now().isoformat()
                    ...
                    messages.success(request, f'Question graded successfully with {marks} marks.')
                else:
...
def course_registration_pdf_view(request, course_id):
    ...
    if not Subject.objects.filter(
        ...
        messages.error(request, "You are not authorized to generate registration forms for this course.")
        return redirect('assessment:grade_results')
    ...
    # Student information
    student = taken_courses.first().student  # Get first student (assuming one student per request)

    info_data = [
        [f"Registration Number: {student.user.username.upper()}", f"Level: {getattr(student, 'level', 'N/A')}"],
        [f"Name: {student.user.get_full_name().upper()}", f"Session: {current_session.session}"],
    ]
    ...
    course_table = Table(course_data, colWidths=[1*cm, 3*cm, 8*cm, 2*cm, 4*cm])
    ...
    signature_data = [
        ...
        ["Lecturer Signature:", "_______________________________", "Date:", "________________"],
        ["Department Head:", "_______________________________", "Date:", "________________"],
    ]
...
def quiz_bank_detail(request, quiz_bank_id):
    ...
    if completed_attempts > 0:
        avg_score = QuizAttempt.objects.filter(
            question_bank=quiz_bank, complete=True
        ).aggregate(avg=Avg('current_score'))['avg'] or 0
    ...
    if request.method == 'POST':
        ...
        if action == 'answer':
            ...
            if current_question.question_type == 'multiple_choice':
                ...
                if selected_options:
                    correct_options = set(current_question.get_correct_options().values_list('id', flat=True))
            ...
            if not next_question:
                ...
                messages.success(request, f'Quiz completed! Score: {attempt.get_percent_correct}%')
...
def add_score_for(request, course_id):
    ...
    if not Subject.objects.filter(
        ...
        messages.error(request, "You are not authorized to enter scores for this course.")
        return redirect('assessment:add_score')
    ...
    if request.method == "POST":
        ...
        for student_id in student_ids:
            ...
            try:
                assignment = Decimal(assignment_score) if assignment_score else Decimal('0.00')
                mid_exam = Decimal(mid_exam_score) if mid_exam_score else Decimal('0.00')
                quiz = Decimal(quiz_score) if quiz_score else Decimal('0.00')
                attendance = Decimal(attendance_score) if attendance_score else Decimal('0.00')
                final_exam = Decimal(final_exam_score) if final_exam_score else Decimal('0.00')
            except (ValueError, TypeError):
                messages.error(request, f"Invalid score format for student {student.user.get_full_name()}")
                continue
...
def assessment_results(request):
    ...
    ).select_related('student__user', 'academic_class').order_by('-gpa')
...
def grade_results(request):
    ...
    taken_courses = TakenCourse.objects.filter(
        ...
        academic_session=current_session
    ).select_related('student__user', 'course').order_by('student__user__last_name')
...
def result_sheet_pdf_view(request, course_id):
    ...
    Story.append(Paragraph(f"<b>Lecturer:</b> {request.user.get_full_name()}", info_style))
    Story.append(Paragraph(f"<b>Session:</b> {current_session.session}", info_style))
    Story.append(Paragraph(f"<b>Total Students:</b> {taken_courses.count()}", info_style))
    Story.append(Spacer(1, 0.5*cm))

    # Results table
    table_data = [
        ["S/N", "Student ID", "Full Name", "Assignment", "Mid Exam", "Quiz", "Attendance", "Final Exam", "Total", "Grade", "Comment"]
    ]
    ...
    # Column widths
    col_widths = [0.8*cm, 2.5*cm, 4*cm, 2*cm, 2*cm, 1.5*cm, 2*cm, 2*cm, 2*cm, 1.5*cm, 2*cm]
...
def quiz_bank_list(request):
    ...
    if is_teacher:
        ...
        for quiz_bank in quiz_banks:
            ...
            best_score = 0
            if completed_attempts > 0:
                best_score = max(attempt.get_percent_correct for attempt in student_attempts.filter(complete=True))
...
def quiz_bank_detail(request, quiz_bank_id):
    ...
    if quiz_bank.single_attempt:
        ...
        if existing_attempt:
            messages.info(request, 'You have already completed this quiz. You cannot attempt it again.')
            return redirect('assessment:student_progress')
    ...
    context = {
        ...
        'can_attempt': not (quiz_bank.single_attempt and student_attempts.filter(complete=True).exists())
    }
    return render(request, 'assessment/quizzes/quiz_bank_detail.html', context)
