...
def get_recent_announcements(context, user, request, limit=5):
    ...
    if hasattr(user, 'student_profile') or user.user_roles.filter(role__role_type='student').exists():
        # Students see general, student-specific, and class-specific announcements
        student_profile = getattr(user, 'student_profile', None)
        class_ids = []
        if student_profile:
            class_ids = list(student_profile.enrollments.values_list('class_enrolled', flat=True))
    ...
    elif user.is_staff or user.user_roles.filter(role__role_type__in=['admin', 'principal', 'super_admin']).exists():
